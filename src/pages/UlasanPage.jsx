...
const UlasanPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchFeedback = async () => {
            try {
                ...
                console.log('Feedback data:', response.data.data); // Debug log
    ...
    const uniqueMenus = useMemo(() => {
        ...
        allFeedback.forEach(fb => {
            ...
            menu_ratings.forEach(mr => {
                // PERBAIKAN: Gunakan menuName (camelCase) konsisten dengan database
                const menuName = mr.menuName;
                if (menuName) menuSet.add(menuName);
            });
        });
        const menuArray = Array.from(menuSet).sort();
        console.log('Unique menus found:', menuArray); // Debug log
        return menuArray;
    }, [allFeedback]);
    ...
    const filteredAndSortedReviews = useMemo(() => {
        ...
        if (filterMenu !== 'all') {
            reviews = reviews.filter(fb => {
                ...
                const hasMenu = menu_ratings.some(mr => {
                    // PERBAIKAN: Gunakan menuName konsisten
                    return mr.menuName === filterMenu;
                });
                console.log(`Checking feedback ${fb.id} for menu "${filterMenu}":`, hasMenu, menu_ratings); // Debug log
                return hasMenu;
            });
        }
        ...
        reviews.sort((a, b) => {
            ...
            return new Date(bDate) - new Date(aDate); // newest
        });
        ...
        return (
            <div className="bg-soft-white min-h-screen flex items-center justify-center">
                <div className="text-center p-12">
                    ...
                    <p className="mt-3 text-charcoal text-sm">Memuat ulasan...</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="container mx-auto max-w-7xl">
                        ...
                        <div className="bg-gradient-to-r from-wood-brown to-light-brown rounded-lg p-6 mb-6 text-white">
                            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                                <div className="text-center md:text-left">
                                    <h1 className="font-serif font-black text-2xl md:text-4xl">Ulasan Pengunjung</h1>
                                    {averageRating > 0 && (
                                        <div className="mt-2 flex items-center justify-center md:justify-start gap-2">
                                            <Star size={20} className="text-yellow-400" fill="currentColor" />
                                            <span className="text-xl font-bold">{averageRating}</span>
                                            <span className="text-white/80 text-sm">
                                                / 5.0 dari {allFeedback.filter(f => {
                                                    const rating = f.cafeRating || f.cafe_rating;
                                                    return rating > 0;
                                                }).length} ulasan
                                            </span>
                                ...
                                {averageRating > 0 && (
                                    <div className="hidden md:block bg-white/10 rounded-lg p-4 backdrop-blur-sm">
                                        <h3 className="text-sm font-semibold mb-2">Distribusi Rating</h3>
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6 shadow-sm sticky top-20 z-40">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-sm font-semibold text-gray-700">Filter & Urutkan</span>
                                <span className="text-xs bg-gray-100 px-2 py-1 rounded-full">{filteredAndSortedReviews.length} hasil</span>
                                {uniqueMenus.length > 0 && (
                                    <span className="text-xs bg-blue-100 px-2 py-1 rounded-full">{uniqueMenus.length} menu tersedia</span>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                ...
                                <div>
                                    <select 
                                        value={filterMenu} 
                                        onChange={e => {
                                            console.log('Menu filter changed to:', e.target.value); // Debug log
                                            setFilterMenu(e.target.value);
                                        }} 
                                        className="w-full p-2 text-sm border border-gray-300 rounded-md bg-white focus:ring-2 focus:ring-wood-brown/20"
                                    >
                                        <option value="all">🍽️ Semua Menu</option>
                            ...
                            {filteredAndSortedReviews.length > 0 ? (
                                ...
                                <div className="lg:col-span-2 xl:col-span-3">
                                    <div className="text-center bg-gray-50 p-8 rounded-lg border-2 border-dashed border-gray-300">
                                        ...
                                        <h3 className="text-lg font-semibold text-gray-700 mb-2">Tidak ada ulasan ditemukan</h3>
                            ...
                            <div className="mt-8 p-4 bg-gray-50 rounded-lg">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-600">Total Ulasan</div>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-600">Menu Diulas</div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-600">Rating 4-5 ⭐</div>
                                    </div>
                                </div>
                            </div>
                        )}
