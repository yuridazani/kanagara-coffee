...
const CompactFeedbackCard = ({ feedback }) => {
                ...
                <div className="w-full min-w-0">
                    ...
                    {feedback.menu_ratings && feedback.menu_ratings.length > 0 && (
                        <div className="flex items-center gap-1 mt-2">
                            ...
                            <span className="text-xs text-charcoal/60">{feedback.menu_ratings.length} menu dinilai</span>
                        </div>
                    )}
                </div>
            </div>
        ...
        <section id="feedback" className="py-12 px-4 bg-soft-white">
            <div className="container mx-auto max-w-6xl">
                ...
                <div className="text-center mb-8" data-aos="fade-up">
                    <h2 className="font-serif font-black text-3xl text-wood-brown">Apa Kata Mereka</h2>
                    <p className="mt-2 text-charcoal/80">Bergabunglah dengan ribuan pelanggan puas lainnya</p>
                    {averageRating > 0 && (
                ...
                <div className="grid lg:grid-cols-2 gap-8" data-aos="fade-up">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="font-serif font-bold text-xl text-wood-brown">Ulasan Terbaik</h3>
                            <Link 
                                to="/ulasan" 
                        ...
                        {topFeedback.length > 0 ? (
                            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                                {topFeedback.map(fb => <CompactFeedbackCard key={fb.id} feedback={fb} />)}
                            </div>
                        ) : (
                    ...
                    <div className="lg:sticky lg:top-4">
                        <h3 className="font-serif font-bold text-xl text-wood-brown mb-4">Bagikan Pengalaman Anda</h3>
                        
                        {isSubmitted ? (
                            <div className="text-center py-8 bg-cream rounded-lg shadow-sm">
                                <CheckCircle size={40} className="text-leaf-green mx-auto mb-3" />
                                <h4 className="font-bold text-lg text-wood-brown">Terima Kasih!</h4>
                                <p className="mt-1 text-sm text-charcoal/80">Ulasan Anda sangat berarti bagi kami</p>
                            </div>
                        ) : (
                            <form onSubmit={handleSubmit} className="bg-cream p-6 rounded-lg shadow-sm space-y-4">
                                ...
                                <div className="space-y-3">
                                    <div>
                                        <label htmlFor="name" className="text-sm font-medium">Nama <span className="text-xs text-charcoal/60">(opsional)</span></label>
                                        <input 
                                            type="text" 
                                            id="name" 
                                            value={formData.name} 
                                ...
                                {/* Rating Cafe */}
                                <div>
                                    <label className="text-sm font-medium">Rating Kafe *</label>
                                    <div className="mt-2">
                                        <StarRatingInput 
                                            rating={cafeRating} 
                                ...
                                {/* Comment */}
                                <div>
                                    <label htmlFor="cafeComment" className="text-sm font-medium">Komentar *</label>
                                    <textarea 
                                        id="cafeComment" 
                                        rows="3" 
                                        value={cafeComment} 
                                ...
                                <div>
                                    <label className="text-sm font-medium flex items-center gap-1">
                                        <Camera size={14}/> Foto <span className="text-xs text-charcoal/60">(opsional)</span>
                                    </label>
                                    <input 
                                        type="file" 
                                        accept="image/*" 
                                ...
                                <div className="border-t pt-4">
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium">Rating Menu (opsional)</span>
                                        <button 
                                            type="button" 
